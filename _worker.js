...
// 源端口：TXT/CSV/API中有明确端口就保留；没有明确端口时回落到443。
function 格式化IP端口(ip, port = '443') {
	ip = String(ip || '').trim();
	port = String(port || '443').trim() || '443';
	if (!ip) return '';
...
function 解析优选地址项(原始项) {
	...
	let addressid = '';
	let port = '-1';
	if (!raw) return { address: '', port, addressid: '' };

	const hashIndex = raw.indexOf('#');
	if (hashIndex >= 0) {
		addressid = raw.slice(hashIndex + 1).trim();
		raw = raw.slice(0, hashIndex).trim();
	}

	let address = raw;

	// 标准IPv6格式：[IPv6] 或 [IPv6]:port
	if (raw.startsWith('[')) {
		const m = raw.match(/^\[([^\]]+)\](?::(\d+))?$/);
		if (m) {
			address = `[${m[1]}]`;
		...
		if (colonCount === 1) {
			...
			// 裸IPv6视为“没有明确端口”，避免把IPv6最后一段误判成端口
			address = `[${raw}]`;
		}
	}

	if (!addressid) addressid = address.replace(/^\[|\]$/g, '');
	return { address, port, addressid };
}

function 应用TLS端口模式(port, portMode) {
	if (portMode === '443') return '443';
	return (!port || port === '-1') ? '443' : String(port);
}
		...
		for (const [index, response] of responses.entries()) {
			...
			if (response.status === 'fulfilled') {
				...
				if (lines[0].split(',').length > 3) {
					...
					const portColumnIndex = header.findIndex(col => /^(端口|port)$/i.test(col));
					const ipColumnIndex = Math.max(0, header.findIndex(col => /^(IP地址|IP|address|地址)$/i.test(col)));

					for (let i = 1; i < lines.length; i++) {
						const row = lines[i].split(',').map(col => col.trim());
						const ip = row[ipColumnIndex] || row[0];
						if (ip) {
							// CSV自身有明确端口时优先使用；否则使用API URL的port参数；再否则443。
							const csvPort = portColumnIndex >= 0 && /^\d+$/.test(row[portColumnIndex] || '')
								? row[portColumnIndex]
								: 测速端口;
							const formatted = 格式化IP端口(ip, csvPort);
							newapi += `${formatted}${节点备注 ? `#${节点备注}` : ''}\n`;
							if (api[index].includes('proxyip=true')) proxyIPPool.push(formatted);
						}
					}
				} else {
		...
		try {
			...
			return dataRows
				...
				.map(row => {
					...
					const formattedAddress = `${格式化IP端口(ipAddress, port)}#${dataCenter}`;

					// 处理代理IP池
					if (csvUrl.includes('proxyip=true') &&
						row[tlsIndex].toUpperCase() === 'TRUE' &&
						!httpsPorts.includes(port)) {
						proxyIPPool.push(格式化IP端口(ipAddress, port));
					}
			...
			<body>
				...
				<div class="container">
					...
					<div style="display:flex; gap:10px; margin-bottom:1.5rem;">
						<button style="margin-bottom:0;" onclick="generateLink('443')">生成订阅（统一443）</button>
						<button style="margin-bottom:0;" onclick="generateLink('source')">生成订阅（源端口优先）</button>
					</div>
				...
				<script>
					...
					function generateLink(portMode = 'source') {
						...
						try {
							...
							if (isVMess){
								...
								subLink = \`https://\${domain}/sub?host=\${host}&uuid=\${uuid}&path=\${encodeURIComponent(path)}&sni=\${sni}&type=\${type}&alpn=\${encodeURIComponent(alpn)}&alterid=\${alterId}&security=\${security}&portmode=\${portMode}\`;
							} else {
								const uuid = link.split("//")[1].split("@")[0];
								const search = link.split("?")[1].split("#")[0];
								const domain = window.location.hostname;
								
								subLink = \`https://\${domain}/sub?\${uuidType}=\${uuid}&\${search}&portmode=\${portMode}\`;
							}
...
export default {
	async fetch(request, env) {
		...
		const portModeRaw = (url.searchParams.get('portmode') || 'source').toLowerCase();
		const portMode = portModeRaw === '443' ? '443' : 'source';
		let host = "";
		let uuid = "";
		let path = "";
		let sni = "";
		...
		if (!isSubConverterRequest && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
			...
			const responseBody = uniqueAddresses.map(address => {
				const parsedAddress = 解析优选地址项(address);
				address = parsedAddress.address;
				let port = 应用TLS端口模式(parsedAddress.port, portMode);
				let addressid = parsedAddress.addressid;

				//console.log(address, port, addressid, portMode);
